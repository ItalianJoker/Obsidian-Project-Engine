...
function prince2Template(): string {
	return [
		...
		"### Registers",
		"",
		"Operational registers live under `Registers/` as Graph-linked Markdown notes:",
		"",
		"- **Risk Register** — entry notes (`R-###`) with status, owner, severity",
		"- **Issue & Change Log** — entry notes (`I-###`) with type, priority, decision",
		"- **Quality Register** — entry notes (`Q-###`) with product, method, result",
		"",
		"Use the project Dashboard **Registers** widgets to quick-add entries.",
		"",
		"### Work Packages",
		"",
		"- Authorise packages per stage when work starts (no empty stubs).",
		"",
	].join("\n");
}
