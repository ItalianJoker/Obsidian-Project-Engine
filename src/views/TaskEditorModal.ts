/**
 * Task editor: recursive nested subtasks, dependencies with cycle detection,
...
export class TaskEditorModal extends Modal {
	...
	private addFlags(): void {
		const wrap = this.contentEl.createDiv({ cls: "pe-field pe-flag-row" });
	...
	private addScheduleActions(): void {
		const wrap = this.contentEl.createDiv({ cls: "pe-inline-row pe-schedule-actions" });
...
export async function openTaskEditorForActiveProject(
	...
	new TaskEditorModal(plugin.app, plugin, projectId, projectLink, null, parentId).open();
}
